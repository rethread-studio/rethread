...
class Images {
    ...
    setImages(img) {
        ...
        const pixelImage = this.createFilledImage(img, { x: 100, y: 100 }, size, pixelSampleSize);
        this.images.push(pixelImage);
        let imgRef = pixelImage;
        this.filters.forEach(_filter => {
            const pos = imgRef.getPosition();
            const emptyImg = this.createEmptyImage({ x: pos.x + 300, y: pos.y }, size, pixelSampleSize, imgRef.getCompleteImage(), _filter);
            imgRef = emptyImg;
