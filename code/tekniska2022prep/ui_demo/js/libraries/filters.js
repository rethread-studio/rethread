...
// No adjustment
function grayscaleFilterous(pixels) {
    let d = pixels;
...
function sepiaFilterous(pixels, adj) {
    let d = pixels;
    for (let i = 0; i < d.length; i += 4) {
        let r = d[i], g = d[i + 1], b = d[i + 2];
...
// No adjustment
function invertFilterous(pixels, adj) {
    let d = pixels;
    for (let i = 0; i < d.length; i += 4) {
        d[i] = 255 - d[i];
        d[i + 1] = 255 - d[i + 1];
...
function brightnessFilterous(pixels, adj) {
    let d = pixels;
    adj = (adj > 1) ? 1 : adj;
    adj = (adj < -1) ? -1 : adj;
    adj = ~~(255 * adj);
...
function hueSaturationFilterous(pixels, adj) {
    let d = pixels;
    for (let i = 0; i < d.length; i += 4) {
...
function saturationFilterous(pixels, adj) {
    let d = pixels;
    adj = (adj < -1) ? -1 : adj;
    for (let i = 0; i < d.length; i += 4) {
        let r = d[i], g = d[i + 1], b = d[i + 2];
...
function contrastFilterous(pixels, adj) {
    adj *= 255;
    let d = pixels;
...
function colorFilterFilterous(pixels, rgbColor) {
    let d = pixels;
    let adj = rgbColor[3];
    for (let i = 0; i < d.length; i += 4) {
...
// RGB Adjust
function rgbAdjustFilterous(pixels, rgbAdj) {
    let d = pixels;
...
function convoluteFilterous(pixels, weights) {
