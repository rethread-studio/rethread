...
module.exports = function (packet) {
    ...
    if (hostContains(packet, 'google') || hostContains(packet, 'gstatic') || hostContains(packet, '1e100.net')) {
        output.add('Google')
    }
    if (hostContains(packet, 'googlemail')) {
        output.add('Gmail')
    }
    ...
    if (ipInRange(packet, '52.119.128-255.0-255') || ipInRange(packet, '52.218.204.0/24') || ipInRange(packet, '52.84-95.0-255.0-255') || ipInRange(packet, '54.240-255.0-255.0-255')) {
        output.add('Amazon')
    }
    ...
    if (hostContains(packet, 'slack') || hostContains(packet, 'ec2-3-120-198-117')) {
        output.add('Slack')
        output.add('Amazon')
    }
