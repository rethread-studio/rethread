...
struct AllPassCascade {
    ...
    ap_filters: [[Biquad<f32>; NUM_BANDS - 2]; NUM_BANDS - 1],
...
impl IirCrossover {
    ...
    pub fn process(
        ...
        num_bands: usize,
        main_io: Sample,
        mut band_outputs: [&mut Sample; NUM_BANDS],
    ) {
        assert!(num_bands >= 2);
        assert!(num_bands <= NUM_BANDS);
        // Required for the SIMD, so we'll just do a hard assert or the unchecked conversions will
        // be unsound

        let mut samples: f32 = main_io;
        match self.mode {
            IirCrossoverType::LinkwitzRiley24 => {
                    ...
                    .enumerate()
                {
                    ...
                    **band_channel_samples = lp_samples;
                ...
                *band_outputs[num_bands - 1] = samples;
            }
        }
    }
...
impl Crossover {
    ...
    pub fn process_lr24(&mut self, samples: f32) -> (f32, f32) {
    ...
    pub fn update_coefficients(
        &mut self,
        lp_coefs: BiquadCoefficients<f32>,
        hp_coefs: BiquadCoefficients<f32>,
    ) {
...
impl AllPassCascade {
    ...
    pub fn compensate_lr24(&mut self, lp_samples: f32, band_idx: usize) -> f32 {
