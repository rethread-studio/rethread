...
export const ScoreList = ({ clickHandler }: React.PropsWithChildren<IScoreInterface>) => {
    ...
    const getUserEvents = () => {
        ...
        for (const key of Object.keys(userEvents)) {
            e.push(
                <div key={uuidv4()} className="flex flex-row justify-between text-gray-200 px-4 lowercase text-sm">
                    <div>{key}</div>
    ...
    return <div className="h-full w-full p-4 fixed top-0 left-0 bg-gray-900 z-40">
        <div className="h-full border-2 border-gray-600 relative flex flex-col justify-start overflow-x-hidden">
            <div className="flex flex-row justify-between content-center pt-2">
                <div className={`text-neon "text-2xl" normalcase pl-6 pt-2 mb-2`}>Your score: {userScore}</div>
            ...
            <div className="px-4 text-3xl mb-2">
                {isLoading ? getUserEvents() : <></>}
            </div>

            <div className={`w-full text-neon "text-2xl" normalcase pl-4 pt-2 mb-4`}>
            ...
            <div className="flex flex-row justify-between mt-4">
                ...
                <a href="https://www.youtube.com/watch?v=dQw4w9WgXcQ&ab_channel=RickAstley" target="_blank" rel='noreferrer' style={{ backgroundColor: "#f6d465" }} className={`cursor-pointer h-20 text-base  w-2/6 text-center transition-all duration-200 normal-case py-2 mb-6 mx-auto place-self-end z-20 text-gray-900 font-light `}>
                    Claim<br></br>award
                </a>
