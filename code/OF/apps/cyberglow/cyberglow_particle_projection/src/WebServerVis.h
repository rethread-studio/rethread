...
public:
  ...
  vector<int> activity_received; // tracks the number of frames since activity
                                 // was received
  vector<vector<int>> activity_received_dots; // tracks the number of frames
                                              // since activity was received
  float radius = 400.0;
  ...
  void change_mode() {
    if (last_vis_mode > 1) {
      ...
      do {
        new_mode =
            static_cast<WebServerVisMode>(floor(ofRandom(last_vis_mode)));
      } while (new_mode == vis_mode);
      vis_mode = new_mode;
    }
  }

  void change_mode(WebServerVisMode mode) { vis_mode = mode; }
  void update() {
    ...
    for (int i = 0; i < activity.size(); i++) {
      activity_smoothed[i] =
          activity_smoothed[i] * (1. - smooth_r) + activity[i] * smooth_r;
    }
    ...
    case WebServerVisMode::LINES: {
      ...
      for (int i = 0; i < activity.size(); ++i) {
        if (activity_smoothed[i] > 0.01) {
          ...
          float length =
              min(activity_smoothed[i], 1.0f) * float(width) * max_line_length;
          float x = length - width * 0.5 * max_line_length;
          ofSetColor(col);
          ofPolyline poly;
          poly.addVertex(width * -0.5 * max_line_length, y);
          poly.addVertex(x, y);
          poly.draw();
        }
      }
    ...
    case WebServerVisMode::DOT: {
      ...
      for (int i = 0; i < activity_received_dots.size(); ++i) {
        ...
        for (int k = event.size() - 1; k >= 0; k--) {
          ...
          ofSetColor(col);
          ofDrawCircle(x - width * 0.5, y, dotSize);
        }
      }
      break;
    }
