...
class EventLine {
    public:
        ...
        EventLine(glm::vec2 position, ofColor color, float max_length): position(position), color(color), max_length(max_length) {
            length = ofRandom(0, max_length);
        }

        void update() {
            length += length_growth;
            if( length < 0.0 || length > max_length ) length_growth *= -1.0;
        }

        void draw(ofxLaser::Manager &laser, glm::vec2 offset) {
            laser.drawLine(position + offset, position + offset + glm::vec2(0, length), color, OFXLASER_PROFILE_FAST);
        }
    ...
    public:
        ...
        EventLineColumn(glm::vec2 offset, int width, int height): offset(offset) {
            ...
            for(int i = 0; i < num_lines; i++) {
                ...
                glm::vec2 pos;
                do {
                    pos = glm::vec2(ofRandom(width*margin, width*(1.0-margin)), ofRandom(0, height - length));
                    overlaps = false;
                    for(auto& l : lines) {
        ...
        void draw(ofxLaser::Manager &laser, float scan_x, float scan_width) {
            for(auto& l : lines) {
                float x = l.position.x+offset.x;
                if(x < scan_x && x > scan_x-scan_width) l.draw(laser, offset);
            }
        }
