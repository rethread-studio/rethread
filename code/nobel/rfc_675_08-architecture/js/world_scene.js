class WorldScene extends Scene {
  constructor() {
    ...
    this.test_country = ["Sweden", "Norway", "Netherlands", "United States"];
  ...
  registerPacket(internalData, country, continent) {
    if (
      this.dashBoard != null &&
      internalData.out == true &&
      this.isInCountries(
        internalData.local_location.country,
        this.selectedRegion
      )
    ) {
...
class DashBoard {
  ...
  writePackages() {
    ...
    text(
      this.packages,
      this.positions.row.r3,
      this.positions.col.c2 + this.positions.padding.top
    );
  }
  ...
  writeSize() {
    ...
    text(
      this.size,
      this.positions.row.r3,
      this.positions.col.c2 + 24 * subsampling
    );
  }
  ...
  writeLocation() {
    ...
    text(
      "LIVE",
      this.positions.row.r1 - 10 * subsampling,
      this.positions.col.c2 + this.positions.padding.top
    );
    circle(
      this.positions.row.r1 - 31 * subsampling,
      this.positions.col.c2 + this.positions.padding.top,
      7 * subsampling
    );
    text(
      this.focuLocation,
      this.positions.row.r1 - 10 * subsampling,
      this.positions.col.c2 + this.positions.padding.top * 2 + 4 * subsampling
    );
  }
...
class CountryManager {
  ...
  addPackage(name) {
    if (this.getCountry(name) != undefined) {
      ...
      if (
        country != undefined &&
        country.country.getPackagesNumb() % country.country.getBigPacket() == 0
      ) {
  ...
  display() {
    ...
    for (let i = 0; i < this.toRender.length; i++) {
      ...
      if (this.toRender[i].country.state == "REMOVE")
        toRemove.push(this.toRender[i].name.name);
    }
  ...
  removecountry(name) {
    const index = this.toRender.findIndex(
      (country) => country.name.name == name
    );
    this.toRender[index].country.refPoint.used = false;
    for (let i = 0; i < this.packages.length; i++) {
      if (
        this.packages[i].countryName == this.toRender[index].country.countryName
      ) {
...
class Country {
  ...
  addVelocity() {
    this.maxspeed =
      this.maxspeed - 0.5 * subsampling < 0.4 * subsampling
        ? 0.4 * subsampling
        : this.maxspeed - 0.5 * subsampling;
  ...
  display() {
    ...
    if (this.state == "MOVE") {
      ...
      const size = this.beatType
        ? sin(Date.now() / this.beat) * this.size
        : cos(Date.now() / this.beat) * this.size;
...
class Package {
  ...
  display() {
    ...
    if (this.state == "MOVE" || this.state == "VANISH") {
      ...
      translate(this.position.x, this.position.y);
      rotate(this.theta);
      let size = this.beatType
        ? sin(Date.now() / this.beat) * this.r
        : cos(Date.now() / this.beat) * this.r;
