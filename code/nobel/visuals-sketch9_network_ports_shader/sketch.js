...
class Node {
  ...
  drawNodes(g) {
    g.fill(this.activity * 9, 70, (1.0-Math.pow(1.0-this.activity, 2.0)) * 80, 100);
    g.noStroke();
    if(this.isWindow == false) {
      g.ellipse(this.pos.x + this.offsetPos.x, this.pos.y + this.offsetPos.y, 10*subsampling, 10*subsampling);
    } else {
...
function setup() {
  ...
  let openVPNNode = new Node(["Privacy", "OpenVPN"], [1194], createVector(0, 0), true, 3);
  leftTopNode1.createConnection(openVPNNode);
  let telnetNode = new Node(["Maintenance", "Telnet"], [23], createVector(0, 0), true, 4);
  leftTopNode1.createConnection(telnetNode);
  let sshNode = new Node(["Maintenance", "SSH"], [22], createVector(0, 0), true, 0);
  ...
  let httpsNode = new Node(["Web", "HTTPS"], [443], createVector(0, 0), true, 1);
  ...
  let ntpNode = new Node(["Time Sync", "NTP"], [123], createVector(0, 0), true, 5);
  rightTopNode1.createConnection(ntpNode);
  let natTraversalNode = new Node(["Infrastructure", "NAT Traversal"], [4500, 3478], createVector(0, 0), true, 8);
  ...
  let ldapNode = new Node(["Security", "LDAP"], [389], createVector(0, 0), true, 9);
  leftBottomNode1.createConnection(ldapNode);
  let imapsNode = new Node(["E-mail", "IMAPS"], [993], createVector(0, 0), true, 12);
  leftBottomNode2.createConnection(imapsNode);
  let smtpNode = new Node(["E-mail", "SMTP"], [25], createVector(0, 0), true, 13);
  ...
  let mysqlNode = new Node(["Data", "MySQL"], [3306], createVector(0, 0), true, 10); 
  rightBottomNode1.createConnection(mysqlNode)
  let bittorrentNode = new Node(["Transmission", "BitTorrent"], [9000, 6881], createVector(0, 0), true, 11); 
  rightBottomNode1.createConnection(bittorrentNode)
  let torNode = new Node(["Privacy", "Tor"], [9001], createVector(0, 0), true, 14); 
...
function draw() {
  ...
  if(!drawMistyShader) {
    ...
    lightsShader.setUniform('windows1', [windows[0].activity, windows[1].activity, windows[2].activity]);
    lightsShader.setUniform('windows2', [windows[3].activity, windows[4].activity, windows[5].activity]);
    lightsShader.setUniform('windows3', [windows[6].activity, windows[7].activity, windows[8].activity]);
    lightsShader.setUniform('windows4', [windows[9].activity, windows[10].activity, windows[11].activity]);
    lightsShader.setUniform('windows5', [windows[12].activity, windows[13].activity, windows[14].activity]);
    shaderCanvas.clear();
...
new WebSocketClient().onmessage = (data) => {
  ...
  if(originNode != undefined) {
    if(originNode.hasPort(internalData.local_port)) {
      ...
      originNode.passParticleOn(new ConnectionParticle(internalData.local_port));
    }
